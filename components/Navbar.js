...
const Navbar = () => {
  ...
  const handleClickOutside = (event) => {
    if (dropdownRef.current && !dropdownRef.current.contains(event.target)) {
      // Start 3-second timeout to close
      closeTimeoutRef.current = setTimeout(() => {
        setIsOpen(false);
      }, 1000);
    } else {
      // Cancel timeout if clicking inside again
  ...
  return (
    <div className='sticky top-0 z-50 bg-transparent backdrop-blur-md'>
      <div className="container mx-auto px-2 sm:px-4 lg:px-6">
        <div className="flex flex-col  sm:flex-row h-fit items-center justify-between pb-4">
          <div className="flex items-center gap-10 sm:gap-6 lg:gap-10  justify-center py-2">
            <Link className="flex items-center gap-2 sm:gap-3 text-text-primary" href="/">
              <img src="/tea.png" alt="tea png" className='h-10 w-10' />

              <h1 className="text-xl sm:text-2xl font-bold tracking-tight">Get Me A Chai</h1>
            </Link>
          </div>
          <div className="flex  justify-center items-center gap-8  px-2">
            <div>
              <form onSubmit={handleSearch} className="flex items-center">
                <input
                  className="bg-zinc-200 text-zinc-600 font-mono ring-1 ring-zinc-400 focus:ring-2 focus:ring-rose-400 outline-none duration-300 placeholder:text-zinc-600 placeholder:opacity-50 rounded-full px-2 py-1.5 shadow-md focus:shadow-lg focus:shadow-rose-400 dark:shadow-md dark:shadow-purple-500 w-46 sm:w-48 lg:w-72"
                  autoComplete="off"
                ...
                />
                <button
                  className="rounded-full bg-zinc-200 text-zinc-600 font-mono ring-1 ring-zinc-400 focus:ring-2 focus:ring-rose-400 outline-none duration-300 placeholder:text-zinc-600 placeholder:opacity-50 px-1.5 py-1.5 rotate-[45deg] ml-2 shadow-md focus:shadow-lg focus:shadow-rose-400 dark:shadow-md dark:shadow-purple-500 cursor-pointer" type="submit"
            ...
            <div>
              <div className="hidden sm:flex items-center gap-4">
                ...
                <div className='flex items-center justify-center relative'>
                  {session ? (
                    <>
                      <div className="hs-dropdown flex-col  sm:flex">
                        <button id="dropdowndefault" type="button" onBlur={() => { setTimeout(() => { setShowDropdownDesktop(false) }, 200); }} onClick={() => setShowDropdownDesktop(!showDropdownDesktop)} className="hs-dropdown-toggle py-1 ps-1 pe-3 inline-flex items-center relative gap-x-2 text-sm font-medium rounded-full shadow-2xs  cursor-pointer focus:outline-none focus:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none pfpcolor" aria-haspopup="menu" aria-expanded={showDropdownDesktop} aria-label="Dropdown">
                          <span>
                            <img
                              alt="profile picture"
                              className="w-7 h-7 rounded-full"
                              src={creator?.profilepic || '/pfp.svg'}
                              loading="lazy"
                              onError={(e) => {
                          ...
                          <span className="font-medium truncate max-w-40 sm:flex hidden">Account</span>
                          <svg
                            className={`size-4 transition-transform duration-200 ${showDropdownDesktop ? 'rotate-180' : ''}`}
                        ...
                        <div className={`absolute top-10 hs-dropdown-menu transition-[opacity,margin] duration-200 hs-dropdown-open:opacity-100 opacity-0 w-fit right-1 sm:right-0 self-end pfpcolor shadow-md rounded-lg mt-2 p-1 ${showDropdownDesktop ? 'opacity-100 visible' : 'opacity-0 invisible'}`} role="menu" aria-orientation="vertical" aria-labelledby="dropdowndefault">
                          <div className=" space-y-0.5">
                            {hasProfile && username && (
                              <Link
                                className="flex items-center justify-center w-full py-1.5 px-1.5 sm:py-2 sm:px-3 text-sm hover:scale-105 pfpcolor hover:border-y-1 border-black focus:outline-none focus:bg-gray-100"
                                href={`/${username}`}
                              >
                                Your Page
                              </Link>
                            ...
                            <Link
                              ...
                              href="/dashboard"
                            >
                              {hasProfile ? "Update Info" : "Dashboard"}
                            </Link>

                            <Link
                              className="flex items-center justify-center w-full py-1.5 px-1.5 sm:py-2 sm:px-3 text-sm pfpcolor hover:border-y-1 hover:scale-105 border-black focus:outline-none focus:bg-gray-100"
                              href="#"
                              onClick={() => signOut()}
                            >
                              Log Out
                    ...
                    <>
                      <button className="w-[120px]  h-[35px] my-3 flex items-center justify-center rounded-tr-xl  rounded-bl-xl cursor-pointer relative overflow-hidden transition-all duration-200-500 ease-in-out shadow-md logbtn hover:shadow-none"
              ...
              <div className="flex sm:hidden relative " ref={dropdownRef}>
                <button
                ...
                >
                  ...
                  {isOpen ? (
                    ...
                    <svg
                      ...
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      className="w-8 h-8 text-black stroke-[var(--stroke-color)] "
                      viewBox="0 0 24 24"
                    >
                ...
                {/* Dropdown content */}
                <div
                  className={`flex flex-col justify-center items-center absolute right-0 top-15 w-auto bg-gray-300 backdrop-blur-lg rounded-3xl shadow-lg transform transition-opacity duration-300 ${isOpen ? "opacity-100 visible" : "opacity-0 invisible"
                    }`}
                >
                  <ul className="py-2">
                    ...
                    <li className="px-3 hover:bg-gray-100 cursor-pointer">
                      <Link href="/" className="w-[120px]  h-[35px] my-3 flex items-center justify-center rounded-tr-xl  rounded-bl-xl cursor-pointer relative overflow-hidden transition-all duration-200-500 ease-in-out shadow-md logbtn hover:shadow-none">
                        Home
                      </Link>
                    </li>
                    <li className="px-3 hover:bg-gray-100 cursor-pointer">
                      <Link href="/about" className="w-[120px]  h-[35px] my-3 flex items-center justify-center rounded-tr-xl  rounded-bl-xl cursor-pointer relative overflow-hidden transition-all duration-200-500 ease-in-out shadow-md logbtn hover:shadow-none">
                        About
                      </Link>
                    </li>
                    <li className="px-3 hover:bg-gray-100 cursor-pointer">
                      <Link href="/how" className="w-[120px]  h-[35px] my-3 flex items-center justify-center rounded-tr-xl  rounded-bl-xl cursor-pointer relative overflow-hidden transition-all duration-200-500 ease-in-out shadow-md logbtn hover:shadow-none">
                        How It Works?
                      </Link>
                    </li>
                    <li className="px-3 hover:bg-gray-100 cursor-pointer">
                          ...
                          <>
                            <div className="hs-dropdown flex-col  sm:flex ">
                              <button
                                ...
                                className="hs-dropdown-toggle inline-flex items-center gap-1.5 px-2 py-1 text-sm font-medium rounded-full shadow-2xs cursor-pointer focus:outline-none focus:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none pfpcolor w-fit"
                                aria-haspopup="menu"
                                aria-expanded={showDropdownDesktop}
                                aria-label="Dropdown"
                              >
                                <img
                                  alt="profile picture"
                                ...
                                <span className="font-medium flex ">Account
                                </span>
                                <svg
                                  className={`size-4 transition-transform duration-200 ${showDropdownDesktop ? 'rotate-180' : ''}`}
                                  xmlns="http://www.w3.org/2000/svg"
